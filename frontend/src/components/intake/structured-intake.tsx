      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-10 space-y-2">
          ...
          <Section title="A few practical things before we wrap up">
            <div><p className="text-sm font-semibold">Would a pet need to move with them?</p><Choices options={["Yes", "No", "Not sure"]} value={draft.humanIntelligenceV2.independenceProfile.petOwnershipImportance} onChange={(value) => update({ humanIntelligenceV2: { ...draft.humanIntelligenceV2, independenceProfile: { ...draft.humanIntelligenceV2.independenceProfile, petOwnershipImportance: value } } })} /></div>
            <div><p className="text-sm font-semibold">Can they go out independently?</p><Choices options={["Yes", "With support", "No", "Not sure"]} value={draft.humanIntelligenceV2.independenceProfile.abilityToLeaveIndependently} onChange={(value) => update({ humanIntelligenceV2: { ...draft.humanIntelligenceV2, independenceProfile: { ...draft.humanIntelligenceV2.independenceProfile, abilityToLeaveIndependently: value } } })} /></div>
            <Field label="What worries them most about moving?" value={draft.humanIntelligenceV2.transitionRiskProfile.biggestFear} onChange={(value) => update({ humanIntelligenceV2: { ...draft.humanIntelligenceV2, transitionRiskProfile: { ...draft.humanIntelligenceV2.transitionRiskProfile, biggestFear: value } } })} placeholder="Tell me in your own words, or say none or not sure" />
            ...
            {assistance.length > 0 && !assistance.includes("Skilled nursing") ? <div className="rounded-[1.5rem] border border-[#bcd9e7] bg-[#f5fbfe] p-5">
              <p className="text-lg font-semibold text-[#183f55]">There may be two good ways to approach the search.</p>
              <p className="mt-2 text-base leading-7 text-[#527083]">Based on what you’ve told me, a more independent senior community may work today if the extra help needed — such as bathing or medication support — is provided by an outside care provider. The alternative is a community that provides that support itself and can potentially increase care later. A more independent setting may preserve more freedom now, but if care needs become significantly greater, another move could eventually be necessary.</p>
              <p className="mt-4 text-sm font-semibold">How broadly would you like me to search?</p>
              <Choices options={["Show me both approaches", "Care provided by the community", "Independent living + outside support"]} value={draft.careSearchApproach} onChange={(value) => update({ careSearchApproach: value })} />
            </div> : null}
