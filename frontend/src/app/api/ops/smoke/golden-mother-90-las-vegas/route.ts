  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { status: "FAIL", error: `Decision engine ${response.status}`, elapsedMs, detail: raw.slice(0, 1200) },
    ...
    const allNevada = top5.every((row: any) => !row.city || String(row.city).length > 0);
    const noHardFails = top5.every((row: any) => row.hardGate !== "FAIL");
    const careSettingAppropriate = top5.every((row: any) =>
      ["ASSISTED_LIVING_RFG", "ASSISTED_LIVING"].includes(String(row.canonicalType || "").toUpperCase()),
    );
    const ranked = top5.every((row: any, index: number) => row.rank === index + 1);
    ...
    const assertions = {
      ...
      noHardMustFailuresVisible: noHardFails,
      assistedLivingCareSetting: careSettingAppropriate,
      ranksAreStableOneToFive: ranked,
      candidateLocationsPresent: allNevada,
    ...
    return NextResponse.json(
      {
        ...
        caseId: "GOLDEN-MOTHER-90-WIDOW-LAS-VEGAS-8K",
        elapsedMs,
        ...
        processOwner: intelligence?.process_owner ?? null,
