...
const locationImportanceOptions = ["Yes", "No"];
const maxDistanceOptions = ["10", "20", "30", "50", "100", "Custom"];

const AI_WORKFLOW_MESSAGES = {
  care: "Understanding care needs...",
  mobility: "Understanding mobility...",
  lifestyle: "Learning daily routines...",
  cognition: "Building patient profile...",
  safety: "Matching evidence...",
  location: "Searching verified communities...",
  language: "Calculating best matches...",
  finalizing: "Preparing recommendations...",
  ready: "Recommendation Ready",
} as const;

type ProfileCategory = {
  key: "careNeeds" | "mobility" | "budget" | "lifestyle" | "cognition" | "safety" | "location" | "language" | "religion";
  label: string;
  complete: boolean;
  optional: boolean;
  pendingLabel: string;
...
function hasMeaningfulNotes(value: string): boolean {
...
export default function Home() {
  ...
  const [locationImportant, setLocationImportant] = useState("");
  const [referenceAddress, setReferenceAddress] = useState("");
  const [maxDistanceMiles, setMaxDistanceMiles] = useState("");
  const [customDistanceMiles, setCustomDistanceMiles] = useState("");
  ...
  const mobilityComplete =
    Boolean(primaryAssistanceLevel) ||
    assistanceLevels.some((value) => mobilitySignals.includes(value)) ||
    /mobility|transfer|wheelchair|walker|fall/i.test(notes);

  const safetyComplete =
    Boolean(recentHospitalization || wanderingConcerns || emergencySupportNetwork) ||
    ["24/7 support required", "Skilled nursing care"].includes(primaryAssistanceLevel);

  const locationComplete =
    locationImportant === "No"
      ? true
      : locationImportant === "Yes" &&
        Boolean(referenceAddress.trim()) &&
        Boolean(maxDistanceMiles) &&
        (maxDistanceMiles !== "Custom" || Boolean(customDistanceMiles.trim()));

  const languageComplete = Boolean(preferredSpokenLanguage || nativeLanguage || medicalDiscussionLanguage);

  const profileCategories: ProfileCategory[] = [
    {
      key: "careNeeds",
      label: "Care Needs",
      complete: Boolean(relationship && primaryAssistanceLevel),
      optional: false,
      pendingLabel: "Care Needs",
      missingFields: [
        ...(relationship ? [] : ["Relationship to resident"]),
        ...(primaryAssistanceLevel ? [] : ["Primary assistance level"]),
      ],
      weight: 16,
    },
    {
      key: "mobility",
      label: "Mobility",
      complete: mobilityComplete,
      optional: false,
      pendingLabel: "Mobility",
      missingFields: mobilityComplete ? [] : ["Mobility or transfer support details"],
      weight: 14,
    },
    {
      key: "budget",
      label: "Budget",
      complete: Boolean(budget > 0),
      optional: false,
      pendingLabel: "Budget",
      missingFields: budget > 0 ? [] : ["Monthly budget"],
      weight: 12,
    },
    {
      key: "lifestyle",
      label: "Lifestyle",
      complete: Boolean(socialInteractionFrequency || hobbyParticipation.length > 0 || happinessPreferences.length > 0 || otherInterests.trim()),
      optional: false,
      pendingLabel: "Lifestyle",
      missingFields:
        socialInteractionFrequency || hobbyParticipation.length > 0 || happinessPreferences.length > 0 || otherInterests.trim()
          ? []
          : ["Lifestyle preferences or activities"],
      weight: 12,
    },
    {
      key: "cognition",
      label: "Cognition",
      complete: Boolean(memoryStatus),
      optional: false,
      pendingLabel: "Cognition",
      missingFields: memoryStatus ? [] : ["Cognitive or memory status"],
      weight: 14,
    },
    {
      key: "safety",
      ...
      optional: false,
      pendingLabel: "Safety",
      missingFields: safetyComplete ? [] : ["Safety risk context"] ,
      weight: 12,
    },
    {
      key: "location",
      ...
      optional: false,
      pendingLabel: locationImportant === "No" ? "Location (not required)" : "Location",
      missingFields:
        locationImportant === "No"
          ? []
          : [
              ...(locationImportant ? [] : ["Location importance"]),
              ...(referenceAddress.trim() ? [] : ["Preferred location"]),
              ...(maxDistanceMiles ? [] : ["Maximum travel distance"]),
              ...(maxDistanceMiles === "Custom" && !customDistanceMiles.trim() ? ["Custom distance in miles"] : []),
            ],
      weight: 12,
    },
    {
      ...
      missingFields: languageComplete ? [] : ["Preferred spoken language"],
      weight: 8,
    },
    {
      key: "religion",
      label: "Religion",
      complete: Boolean(religionImportance || faithTraditions.length > 0),
      optional: true,
      pendingLabel: "Religion",
      missingFields: [],
      weight: 0,
    },
  ];

  const requiredCategories = profileCategories.filter((category) => !category.optional);
  const requiredWeightTotal = requiredCategories.reduce((sum, category) => sum + category.weight, 0);
  const requiredCompletedWeight = requiredCategories.reduce((sum, category) => sum + (category.complete ? category.weight : 0), 0);

  const contextBonus =
    (hasMeaningfulNotes(notes) ? 4 : 0) +
    (notes.trim().length >= 80 ? 3 : 0) +
    (medicalDocumentsAvailable ? 3 : 0);

  const aiProgressPercent = clampScore((requiredCompletedWeight / requiredWeightTotal) * 100 + contextBonus);
  const missingRequiredFields = requiredCategories.flatMap((category) => category.missingFields);
  const recommendationReady = missingRequiredFields.length === 0 && aiProgressPercent >= 85;

  const aiThinkingMessage = recommendationReady
    ? AI_WORKFLOW_MESSAGES.ready
    : !profileCategories.find((category) => category.key === "careNeeds")?.complete
      ? AI_WORKFLOW_MESSAGES.care
      : !profileCategories.find((category) => category.key === "mobility")?.complete
        ? AI_WORKFLOW_MESSAGES.mobility
        : !profileCategories.find((category) => category.key === "lifestyle")?.complete
          ? AI_WORKFLOW_MESSAGES.lifestyle
          : !profileCategories.find((category) => category.key === "cognition")?.complete
            ? AI_WORKFLOW_MESSAGES.cognition
            : !profileCategories.find((category) => category.key === "safety")?.complete
              ? AI_WORKFLOW_MESSAGES.safety
              : !profileCategories.find((category) => category.key === "location")?.complete
                ? AI_WORKFLOW_MESSAGES.location
                : !profileCategories.find((category) => category.key === "language")?.complete
                  ? AI_WORKFLOW_MESSAGES.language
                  : AI_WORKFLOW_MESSAGES.finalizing;

  const remainingNeedsSummary =
    missingRequiredFields.length === 1
      ? `We only need ${missingRequiredFields[0].toLowerCase()} to prepare your personalized recommendations.`
      : `We still need ${missingRequiredFields.slice(0, 2).map((field) => field.toLowerCase()).join(" and ")} to prepare your personalized recommendations.`;
  ...
  useEffect(() => {
    ...
    setLocationImportant(String(source.locationImportant || ""));
    setReferenceAddress(String(source.referenceAddress || ""));
    setMaxDistanceMiles(String(source.maxDistanceMiles || ""));
    setCustomDistanceMiles(String(source.customDistanceMiles || ""));
  ...
  const handleFindHome = () => {
    ...
    setState({
      ...
      locationImportant,
      referenceAddress,
      maximumDistanceMiles: maxDistanceMiles,
      customDistanceMiles,
      otherInterests,
      notes: [notes.trim(), otherInterests.trim() ? `Other interests: ${otherInterests.trim()}` : ""].filter(Boolean).join("\n"),
      humanIntelligenceV2: {
        socialProfile: {
          livingAloneDuration,
          socialInteractionFrequency,
        ...
        interestsProfile: [...independenceInterests, ...hobbyParticipation, ...happinessPreferences, ...(otherInterests.trim() ? [otherInterests.trim()] : [])],
        independenceProfile: {
          drivingImportance,
          cookingImportance,
          abilityToLeaveIndependently,
    ...
    if (referenceAddress.trim()) params.set("location", referenceAddress.trim());
    if (maxDistanceMiles && maxDistanceMiles !== "Custom") params.set("distanceMiles", maxDistanceMiles);
    if (maxDistanceMiles === "Custom" && customDistanceMiles.trim()) params.set("distanceMiles", customDistanceMiles.trim());
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#e8f4ef_0%,#f8fbfc_40%,#ffffff_74%)] px-6 py-10 sm:px-10 lg:px-16">
      <section className="mx-auto max-w-6xl">
        <div className="rounded-3xl border border-[#e8dcc9] bg-white/92 p-4 shadow-[0_24px_80px_-38px_rgba(96,80,56,0.38)] backdrop-blur sm:p-6">
          <section className="relative overflow-hidden rounded-3xl border border-[#d9e6e1] bg-[#f2f8f5]">
            <img
              src="/hero-reference.png"
              alt="OPTIME hero reference image"
              className="h-[420px] w-full object-cover object-[62%_center] sm:h-[470px]"
            />
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(240,247,245,0.9)_0%,rgba(240,247,245,0.66)_42%,rgba(240,247,245,0.2)_70%,rgba(240,247,245,0.06)_100%)]" />
            <div className="absolute inset-y-0 left-0 z-10 w-full rounded-3xl bg-[rgba(248,252,251,0.78)] backdrop-blur-[1.5px] sm:w-[64%]" />

            <div className="absolute inset-y-0 left-0 z-20 flex w-full items-center px-6 py-6 sm:w-[64%] sm:px-10 lg:px-12">
              <div className="w-full max-w-2xl">
                <OptimeStaticLogo subtitle="Trusted care guidance" />

                <h1 className="mt-5 text-4xl font-semibold leading-[1.05] text-[#1f392a] sm:text-6xl">We help families make confident decisions.</h1>

                <div className="mt-5 h-1 w-16 rounded-full bg-[#6ea897]" />

                <p className="mt-5 text-lg leading-relaxed text-[#4f5d4d]">
                  Calm, evidence-based guidance that starts building the right recommendation from your first answer.
                </p>

                <div className="mt-7 flex flex-wrap gap-2.5 text-sm">
                  <p className="rounded-full bg-[#5f9585] px-4 py-2 font-semibold text-white">✓ Evidence-based matching</p>
                  <p className="rounded-full bg-[#5f9585] px-4 py-2 font-semibold text-white">✓ Clear confidence levels</p>
                  <p className="rounded-full bg-[#5f9585] px-4 py-2 font-semibold text-white">✓ Personal family guidance</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="mt-6 grid gap-5">
            <aside className="sticky top-20 z-30 rounded-2xl border border-[#d4e5df] bg-white/95 p-4 shadow-[0_14px_36px_-20px_rgba(25,85,73,0.48)] backdrop-blur">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#4b6f6a]">AI Case Understanding</p>
                  <p className="mt-1 text-base font-semibold text-[#2d5d52]">Building your recommendation...</p>
                </div>
                <div className="rounded-full border border-[#d8e6e2] bg-[#f3faf8] px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-[#2c5650]">
                  AI Workflow Status
                </div>
              </div>

              <div className="mt-4 grid gap-3 rounded-xl border border-[#dce9e5] bg-[#f8fcfb] p-3 md:grid-cols-2">
                <div className="space-y-3">
                  <OptimeDynamicLogo progress={aiProgressPercent} ready={recommendationReady} />
                  <p className="text-sm font-medium text-[#305d52]">{aiThinkingMessage}</p>
                  {recommendationReady ? (
                    <div className="rounded-lg border border-[#c9ded8] bg-[#eef9f5] px-3 py-2 text-sm text-[#2f6257]">
                      <p className="font-semibold">✓ Recommendation Ready</p>
                      <p className="mt-1">Your personalized recommendations are ready.</p>
                    </div>
                  ) : (
                    <div className="rounded-lg border border-[#e0ece8] bg-white px-3 py-2 text-sm text-[#345f54]">
                      <p className="font-semibold">Almost ready.</p>
                      <p className="mt-1">We still need:</p>
                      <ul className="mt-1 list-disc space-y-1 pl-5">
                        {missingRequiredFields.slice(0, 4).map((field) => (
                          <li key={field}>{field}</li>
                        ))}
                      </ul>
                      <p className="mt-2">{remainingNeedsSummary}</p>
                    </div>
                  )}
                </div>

                <div className="rounded-lg border border-[#dfebe7] bg-white p-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#54766f]">Live Patient Profile</p>
                  <ul className="mt-2 space-y-1.5 text-sm text-[#355f55]">
                    {profileCategories.map((category) => {
                      const marker = category.optional ? "○" : category.complete ? "✓" : "⏳";
                      const label = category.optional ? `${category.label} (optional)` : category.pendingLabel;
                      return (
                        <li key={category.key}>
                          {marker} {label}
                        </li>
            ...
            <article className="rounded-2xl border border-[#e7ddcd] bg-[#fffefb] p-5">
              ...
              <p className="mt-1 text-sm text-[#6c6358]">
                Recommendations automatically include communities approximately +/-20% around your selected budget.
                Within this tolerance, matching does not penalize candidates only because of price.
              </p>
              <div className="mt-5">
                <input
                  type="range"
            ...
            <article className="rounded-2xl border border-[#e7ddcd] bg-[#fffefb] p-5">
              <h3 className="text-lg font-semibold text-[#2f2a24]">8. Lifestyle, Activities &amp; Personal Interests</h3>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              <p className="mt-4 text-sm font-medium text-[#5e5346]">What would make {relationshipLabel} happiest?</p>
              <div className="mt-3 flex flex-wrap gap-2.5">
                {happinessOptions.map((option) => (
                  <OptionChip
                    key={`lifestyle-happy-${option}`}
                    label={option}
              ...
              <div className="mt-4">
                <p className="text-sm font-medium text-[#5e5346]">Other interests</p>
                <input
                  value={otherInterests}
                  onChange={(event) => setOtherInterests(event.target.value)}
                  className="mt-2 w-full rounded-xl border border-[#dfd4c3] px-4 py-3 text-base text-[#52483d] outline-none ring-[#87a79b] transition focus:ring-2"
                  placeholder="Gardening, chess, photography, volunteering..."
                />
              </div>
            </article>

            <article className="rounded-2xl border border-[#e7ddcd] bg-[#fffefb] p-5">
              <h3 className="text-lg font-semibold text-[#2f2a24]">9. Family, culture, and language</h3>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
            ...
            <article className="rounded-2xl border border-[#e7ddcd] bg-[#fffefb] p-5">
              <h3 className="text-lg font-semibold text-[#2f2a24]">10. Cultural intelligence profile (asked for everyone)</h3>
            ...
            <article className="rounded-2xl border border-[#e7ddcd] bg-[#fffefb] p-5">
              <h3 className="text-lg font-semibold text-[#2f2a24]">11. Independence and transition</h3>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
            ...
            <article className="rounded-2xl border border-[#e7ddcd] bg-[#fffefb] p-5">
              <h3 className="text-lg font-semibold text-[#2f2a24]">12. Is location important?</h3>
              <div className="mt-4 flex flex-wrap gap-2.5">
                {locationImportanceOptions.map((option) => (
                  <OptionChip
                    key={`location-important-${option}`}
                    label={option}
                    isActive={locationImportant === option}
                    onClick={() => setLocationImportant(option)}
                  />
                ))}
              </div>
              ...
              {locationImportant === "Yes" ? (
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <p className="text-sm font-medium text-[#5e5346]">Reference Address</p>
                    <input
                      value={referenceAddress}
                      onChange={(event) => setReferenceAddress(event.target.value)}
                      className="mt-2 w-full rounded-xl border border-[#dfd4c3] px-4 py-3 text-base text-[#52483d] outline-none ring-[#87a79b] transition focus:ring-2"
                      placeholder="Street, ZIP, city, or state"
                    />
                  </div>

                  <div className="sm:col-span-2">
                    <p className="text-sm font-medium text-[#5e5346]">Maximum acceptable distance</p>
                    <div className="mt-3 flex flex-wrap gap-2.5">
                      {maxDistanceOptions.map((option) => (
                        <OptionChip
                          key={`distance-${option}`}
                          label={`${option}${option === "Custom" ? "" : " miles"}`}
                          isActive={maxDistanceMiles === option}
                          onClick={() => setMaxDistanceMiles(option)}
                        />
                      ))}
                    </div>
                  </div>

                  {maxDistanceMiles === "Custom" ? (
                    <div>
                      <p className="text-sm font-medium text-[#5e5346]">Custom distance (miles)</p>
                      <input
                        value={customDistanceMiles}
                        onChange={(event) => setCustomDistanceMiles(event.target.value)}
                        className="mt-2 w-full rounded-xl border border-[#dfd4c3] px-4 py-3 text-base text-[#52483d] outline-none ring-[#87a79b] transition focus:ring-2"
                        placeholder="e.g. 35"
                      />
                    </div>
            ...
            <article className="rounded-2xl border border-[#e7ddcd] bg-[#fffefb] p-5">
              <h3 className="text-lg font-semibold text-[#2f2a24]">13. Tell us anything else that would help us find the right community.</h3>
          ...
          <div className="mt-8">
            <button
              ...
              className="w-full rounded-full bg-[#3a8c79] px-6 py-4 text-base font-semibold text-white transition hover:bg-[#2f7464] disabled:cursor-not-allowed disabled:bg-[#9cbfb6] sm:w-auto"
            >
              {recommendationReady ? "View My Recommendations" : `${ctaText} (continue profile)`}
            </button>
          </div>
        </div>
      </section>
