...
SUPPLEMENTAL_AGENT_DEFS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "mission": "Track named competitors' (A Place for Mom, Caring.com, Seniorly) public positioning, monetization language, feature surface, and scale claims via live fetch with real change detection; separately, research their service model, organizational scale, and ownership monthly.",
        "entry_point": "backend/app/services/competitive_intelligence_service.py, backend/app/services/competitor_structural_research_service.py",
        "trigger": "FastAPI startup background scheduler",
        "schedule": f"Homepage tracking every {int(os.getenv('OPTIME_COMPETITIVE_INTEL_INTERVAL_SECONDS', str(6 * 60 * 60))) // 3600} hour(s); structural research every {int(os.getenv('OPTIME_COMPETITOR_RESEARCH_INTERVAL_SECONDS', str(30 * 24 * 60 * 60))) // 86400} day(s); both while the FastAPI process is alive",
        "dependencies": ["Chief AI Supervisor"],
        "expected_outputs": ["Competitive intelligence signals", "Change-detected market moves", "Competitor structural research findings", "Agent knowledge cycle summaries"],
    },
    {
