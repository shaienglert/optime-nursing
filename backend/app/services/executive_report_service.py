...
SUPPLEMENTAL_AGENT_DEFS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "mission": "Track named competitors' (A Place for Mom, Caring.com, Seniorly) public positioning, monetization language, feature surface, and scale claims via live fetch with real change detection.",
        "entry_point": "backend/app/services/competitive_intelligence_service.py",
        "trigger": "FastAPI startup background scheduler",
        "schedule": f"Every {int(os.getenv('OPTIME_COMPETITIVE_INTEL_INTERVAL_SECONDS', str(6 * 60 * 60))) // 3600} hour(s) while the FastAPI process is alive",
        "dependencies": ["Chief AI Supervisor"],
        "expected_outputs": ["Competitive intelligence signals", "Change-detected market moves", "Agent knowledge cycle summaries"],
    },
    {
