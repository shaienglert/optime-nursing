...
def _candidate_official_url(facility_name: str, city: str, canonical_id: str) -> Optional[str]:
    ...
    name_tokens = [t for t in _norm(facility_name).split() if len(t) >= 4][:4]
    for link in links:
        ...
        if not domain or any(domain.endswith(skip) for skip in _SKIP_DOMAINS):
            continue
        haystack = _norm(url + " " + domain)
        if name_tokens and sum(1 for token in name_tokens if token in haystack) < min(2, len(name_tokens)):
            continue
        return url
    return None
...
def _persist_record(db, *, agent_key: str, canonical_id: str, facility_name: str, source_url: str, payload: Dict[str, Any]) -> None:
    ...
    db.add(AgentKnowledgeRecord(agent_key=agent_key, record_type="las_vegas_decision_evidence", entity_key=canonical_id, summary=summary, payload_json=encoded, confidence=0.82 if verified else 0.65, source="OFFICIAL_PROVIDER_WEBSITE" if source_url else "PUBLIC_RESEARCH_NO_VERIFIED_SOURCE"))
