...
This module intentionally does not invent facility lifestyle facts. It converts
already-collected person signals into explicit decision dimensions and combines
them only with evidence-backed facility facts. Missing material evidence remains
UNKNOWN and may produce an adaptive clarification rather than a fabricated score.
"""
...
def _community_size_preference(questionnaire: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if any(token in normalized for token in ("small", "intimate", "few", "home-like", "home like")):
        return {
            "value": "SMALL",
            "source": "questionnaire.humanIntelligenceV2.personalityProfile.communitySizePreference",
            "confidence": 1.0,
        }
...
def _recent_bereavement(questionnaire: Dict[str, Any], natural_language_query: str) -> Dict[str, Any]:
    ...
    for value, source in candidates:
        if _contains_any(value, ("widow", "widower", "bereav", "recent loss", "recently", "spouse died", "spouse passed")):
...
def _social_transition_priority(questionnaire: Dict[str, Any], natural_language_query: str) -> Dict[str, Any]:
    ...
    if bereavement["value"] == "YES":
        # Recent bereavement makes social/transition fit decision-relevant. It does
        # not imply that a larger or smaller community is preferable.
        return {
            "value": "REVIEW_REQUIRED",
            "source": bereavement["source"],
            "confidence": bereavement["confidence"],
            "reason": "Recent bereavement makes social and transition fit material, but preferred community style remains unknown.",
        }
    return {"value": "UNKNOWN", "source": "UNKNOWN", "confidence": 0.0}
...
def _independence_priority(questionnaire: Dict[str, Any], natural_language_query: str) -> Dict[str, Any]:
    ...
    values = [
        profile.get("drivingImportance"),
        profile.get("cookingImportance"),
        profile.get("abilityToLeaveIndependently"),
        profile.get("hostingFamilyImportance"),
    ]
    ...
    nl = _norm(natural_language_query)
    if any(token in nl for token in ("still independent", "independent", "still mobile", "drives himself", "drives herself")):
...
def build_human_intelligence_context(
    ...
    if social_transition["value"] in {"HIGH", "REVIEW_REQUIRED"} and community_size["value"] == "UNKNOWN":
        adaptive_questions.append(
            {
                "question_key": "community_size_preference",
                "question": "Would you prefer a small home-like setting with a few residents, a larger senior community with more people and activities, or no preference?",
                "reason": "Community style can materially change the ranking when social/transition fit is important.",
                "decision_dimensions": ["community_style", "social_transition_fit"],
    ...
    if bereavement["value"] == "YES" and social_transition["value"] == "REVIEW_REQUIRED":
        adaptive_questions.append(
            {
                "question_key": "social_interaction_need_after_loss",
                "question": "Since the loss, would more daily social contact feel helpful, overwhelming, or neither?",
                "reason": "Recent bereavement makes transition and social support relevant without determining the preferred level of social intensity.",
                "decision_dimensions": ["social_transition_fit"],
                "information_gain": "HIGH",
            }
        )

    return {
        "version": "human-intelligence-runtime-v1",
        "signals": {
        ...
        "principles": [
            ...
            "Recent bereavement makes transition/social fit decision-relevant but does not determine community size preference.",
