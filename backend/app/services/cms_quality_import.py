from sqlalchemy.orm import Session

from app.models.facility import QualityMeasure
from app.services.cms_service import (
    CMS_QUALITY_DATASET_ID,
    clean_state,
    download_dataset,
    iter_csv_rows,
    to_float,
)


def import_quality_data(db: Session, ccn_to_facility_id: dict, state: str = "FL") -> dict:
    file_path = download_dataset(CMS_QUALITY_DATASET_ID, "quality_measures.csv")

    db.query(QualityMeasure).delete()
    db.commit()

    imported = 0
    missing_records = 0
    failed_mappings = 0

    for row in iter_csv_rows(file_path):
        if clean_state(row.get("State")) != state:
            continue

        ccn = row.get("CMS Certification Number (CCN)")
        facility_id = ccn_to_facility_id.get(ccn or "")
        if not facility_id:
            failed_mappings += 1
            continue

        measure_code = row.get("Measure Code") or "UNKNOWN"
        measure_name = row.get("Measure Description") or "Unknown measure"
        measure_value = to_float(row.get("Four Quarter Average Score"))

        if measure_value is None:
            missing_records += 1

        db.add(
            QualityMeasure(
                facility_id=facility_id,
                measure_code=measure_code,
                measure_name=measure_name,
                measure_value=measure_value,
                quality_rating=None,
                period_label=row.get("Measure Period") or "Unknown",
            )
        )
        imported += 1

    db.commit()

    return {
        "quality_rows_imported": imported,
        "missing_records": missing_records,
        "failed_mappings": failed_mappings,
    }
