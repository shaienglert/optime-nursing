...
def test_synthetic_pilot_rehab_is_classified_before_client_readiness() -> None:
    ...
    assert discovery["total_facilities_classified"] == 200
    assert discovery["classification_counts"]["REHABILITATION"] == 25
...
def test_catalog_stores_provenance_and_retrieves_rehab_from_tables() -> None:
    ...
    assert query["candidate_count"] == 30
    assert query["verified_capability_match_count"] == 30
    assert query["classification_counts"] == {"REHABILITATION": 25, "SKILLED_NURSING": 5}
    assert query["excluded_explicit_negative_count"] == 170
...
def test_catalog_does_not_treat_missing_capability_as_negative() -> None:
    ...
    assert query["candidate_count"] == 200
    assert query["verified_capability_match_count"] == 0
    assert query["pending_verification_count"] == 200
    assert query["excluded_explicit_negative_count"] == 0


def test_pilot_exposure_limit_does_not_shrink_the_catalog() -> None:
    with patch.dict(
        "os.environ",
        {"OPTIME_CANONICAL_MARKET": "synthetic-pilot", "OOMNIK_PILOT_FACILITY_LIMIT": "50"},
        clear=False,
    ):
        refresh_runtime_cache("test_catalog_and_exposure_are_separate")
        assert len(get_facility_knowledge_catalog()) == 200
        assert len(get_exposed_canonical_facility_ids()) == 50
