...
class SemanticAiLearningCenterContractTests(unittest.TestCase):
    def test_ai_prompt_contains_rules_and_learning_center_advice(self) -> None:
        ...
        def transport(payload):
            ...
            return {
                ...
                "concerns": [],
                "implications": [{"derived_from": ["gluten allergy", "does not cook"], "implication": "Reliable daily gluten-free meal provision may be a MUST", "certainty": "LIKELY", "requires_confirmation": True}],
                "statements": [
                    {"raw_text": "gluten allergy", "meaning": "dietary safety constraint", "importance": "MUST", "knowledge_state": "KNOWN", "status": "USED", "mapped_parameters": ["diet.gluten_free"], "clarification_question": None, "research_task": None},
                    {"raw_text": "does not cook", "meaning": "depends on provided meals", "importance": "MUST", "knowledge_state": "AMBIGUOUS", "status": "ASKED", "mapped_parameters": ["meals.daily_provision"], "clarification_question": "Do you need all daily meals provided by the community?", "research_task": None},
                ],
        ...
        learning = {"advisor": "OPTIME_NURSING_LEARNING_CENTER", "consulted": True, "agent_count": 10, "available_agent_count": 10, "agents": [{"agent_key": "nutrition_intelligence", "status": "AVAILABLE", "knowledge": "gluten-free evidence"}], "policy": {}}
        with patch("app.services.semantic_intent_ai.build_learning_center_advice", return_value=learning):
            result = interpret_client_intent_with_ai(user_text="75-year-old widow, independent, luxury, social, games, gluten allergy, does not cook", questionnaire_state={}, transport=transport)
    ...
    def test_single_asked_statement_can_use_top_level_next_question(self) -> None:
        payload = {
            "statements": [
                {"raw_text": "walker and short internal routes", "meaning": "mobility is decision critical", "importance": "MUST", "knowledge_state": "AMBIGUOUS", "status": "ASKED", "mapped_parameters": ["mobility.internal_route"], "clarification_question": None, "research_task": None},
                {"raw_text": "enjoys music", "meaning": "music preference", "importance": "NICE", "knowledge_state": "KNOWN", "status": "USED", "mapped_parameters": ["activities.music"], "clarification_question": None, "research_task": None},
            ],
            "next_question": "Do you manage bathing, dressing, toileting, transfers, and medications independently?",
            "research_requests": [],
            "decision_readiness": "NEEDS_CLARIFICATION",
        }
        learning = {"advisor": "OPTIME_NURSING_LEARNING_CENTER", "consulted": True, "agent_count": 1, "available_agent_count": 1, "agents": [], "policy": {}}
        with patch("app.services.semantic_intent_ai.build_learning_center_advice", return_value=learning):
            result = interpret_client_intent_with_ai(user_text="I use a walker and can only walk 100 meters", questionnaire_state={}, transport=lambda _: payload)
        self.assertEqual("NEEDS_CLARIFICATION", result["decision_readiness"])
        self.assertEqual(payload["next_question"], result["statements"][0]["clarification_question"])
        self.assertTrue(result["question_trace_normalization"]["applied"])

    def test_missing_asked_question_still_fails_closed_without_top_level_question(self) -> None:
        payload = {
            "statements": [{"raw_text": "walker", "meaning": "mobility", "importance": "MUST", "knowledge_state": "AMBIGUOUS", "status": "ASKED", "mapped_parameters": [], "clarification_question": None, "research_task": None}],
            "next_question": None,
            "decision_readiness": "NEEDS_CLARIFICATION",
        }
        learning = {"advisor": "OPTIME_NURSING_LEARNING_CENTER", "consulted": True, "agent_count": 1, "available_agent_count": 1, "agents": [], "policy": {}}
        with patch("app.services.semantic_intent_ai.build_learning_center_advice", return_value=learning):
            with self.assertRaisesRegex(RuntimeError, "ASKED_WITHOUT_QUESTION"):
                interpret_client_intent_with_ai(user_text="walker", questionnaire_state={}, transport=lambda _: payload)
