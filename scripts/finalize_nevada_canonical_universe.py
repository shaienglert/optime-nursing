...
def cid_business(v): return 'NV-BIZ-'+re.sub(r'[^A-Za-z0-9]+','-',v).strip('-').upper()
...
def cms_only(row,at):
 ...
 r={'canonical_id':cid_cms(c),'facility_name':row.get('Provider Name') or 'UNKNOWN','canonical_type':'SKILLED_NURSING','facility_modalities':['SKILLED_NURSING'],'license_type':'UNKNOWN','nevada_license_id':'UNKNOWN','business_license_id':'UNKNOWN','license_status':'UNKNOWN','address':address,'city':city,'state':'NV','zip':z,'county':county,'phone':row.get('Telephone Number') or 'UNKNOWN','licensed_capacity':'UNKNOWN','administrator':'UNKNOWN','administrator_role':'UNKNOWN','first_issue_date':'UNKNOWN','expiration_date':'UNKNOWN','disciplinary_action':'UNKNOWN','cms_ccn':c,'memory_care_classification':'UNKNOWN','memory_care_evidence':'UNKNOWN','independent_living_classification':'UNKNOWN','independent_living_evidence':'UNKNOWN','official_detail':'UNKNOWN','detail_url':'UNKNOWN','is_clark_county':norm(county)=='clark','is_las_vegas_valley':norm(county)=='clark' and norm(city) in {'las vegas','north las vegas','henderson'},'campus_group_id':campus_id(address,city,z),'source_records':[e],'field_provenance':{},'identity_merge_evidence':[{'method':'CMS_ONLY','cms_ccn':c,'source':'CMS Care Compare'}],'review_flags':['NEVADA_LICENSE_ID_UNKNOWN','MEMORY_CARE_UNKNOWN']}
 for f in ['facility_name','canonical_type','facility_modalities','address','city','state','zip','county','phone','cms_ccn']:r['field_provenance'][f]=e
 ...
 return r
def il_record(row,at):
 lic=row.get('business_license_number') or 'UNKNOWN'; address=row.get('address') or 'UNKNOWN'; city=row.get('city') or 'UNKNOWN'; z=zip5(row.get('zip')); primary=row.get('primary_source_url') or 'UNKNOWN'; eb=ev('City of Las Vegas Business Licenses',LV_BUSINESS_URL,lic,'BUSINESS_LICENSE_IDENTITY',at); ep=ev('Primary operator/provider evidence',primary,lic,'PRIMARY_OPERATOR_OR_PROVIDER_EVIDENCE',at)
 r={'canonical_id':cid_business(lic),'facility_name':row.get('business_name') or 'UNKNOWN','canonical_type':'INDEPENDENT_LIVING','facility_modalities':['INDEPENDENT_LIVING'],'license_type':'BUSINESS_LICENSE_APARTMENT_HOUSE','nevada_license_id':'UNKNOWN','business_license_id':lic,'license_status':row.get('business_license_status') or 'UNKNOWN','address':address,'city':city,'state':row.get('state') or 'NV','zip':z,'county':'Clark' if norm(city) in {'las vegas','north las vegas','henderson'} else 'UNKNOWN','phone':'UNKNOWN','licensed_capacity':'UNKNOWN','administrator':'UNKNOWN','administrator_role':'UNKNOWN','first_issue_date':'UNKNOWN','expiration_date':'UNKNOWN','disciplinary_action':'UNKNOWN','cms_ccn':'UNKNOWN','memory_care_classification':'UNKNOWN','memory_care_evidence':'UNKNOWN','independent_living_classification':'CONFIRMED','independent_living_evidence':{'primary_source_url':primary,'secondary_primary_source_url':row.get('secondary_primary_source_url') or 'UNKNOWN','evidence_summary':row.get('evidence_summary') or 'UNKNOWN','care_services_inferred':False},'official_detail':'UNKNOWN','detail_url':primary,'is_clark_county':norm(city) in {'las vegas','north las vegas','henderson'},'is_las_vegas_valley':norm(city) in {'las vegas','north las vegas','henderson'},'campus_group_id':campus_id(address,city,z),'source_records':[eb,ep],'field_provenance':{},'identity_merge_evidence':[{'method':'EXACT_BUSINESS_LICENSE_PLUS_PRIMARY_PROPERTY_EVIDENCE','business_license_id':lic,'source':'City of Las Vegas + primary property/operator'}],'review_flags':[]}
 for f in ['facility_name','canonical_type','facility_modalities','business_license_id','license_status','address','city','state','zip','county','independent_living_classification','independent_living_evidence','detail_url','is_clark_county','is_las_vegas_valley']:r['field_provenance'][f]=ep if f in {'canonical_type','facility_modalities','independent_living_classification','independent_living_evidence','detail_url'} else eb
 return r
def enrich_existing_with_il(r,row,at):
 lic=row.get('business_license_number') or 'UNKNOWN'; primary=row.get('primary_source_url') or 'UNKNOWN'; eb=ev('City of Las Vegas Business Licenses',LV_BUSINESS_URL,lic,'BUSINESS_LICENSE_IDENTITY',at); ep=ev('Primary operator/provider evidence',primary,lic,'PRIMARY_OPERATOR_OR_PROVIDER_EVIDENCE',at)
 for e in (eb,ep):
  if not any(x.get('source')==e['source'] and x.get('source_record_id')==e['source_record_id'] for x in r['source_records']):r['source_records'].append(e)
 r['business_license_id']=lic;r['independent_living_classification']='CONFIRMED';r['independent_living_evidence']={'primary_source_url':primary,'secondary_primary_source_url':row.get('secondary_primary_source_url') or 'UNKNOWN','evidence_summary':row.get('evidence_summary') or 'UNKNOWN','care_services_inferred':False};mods=set(r.get('facility_modalities') or [r.get('canonical_type')]);mods.add('INDEPENDENT_LIVING');r['facility_modalities']=sorted(m for m in mods if present(m));r['identity_merge_evidence'].append({'method':'EXACT_NORMALIZED_NAME_ADDRESS_CITY_ZIP_IL_ENRICHMENT','business_license_id':lic,'source':'City of Las Vegas + primary property/operator'});r['field_provenance']['independent_living_classification']=ep;r['field_provenance']['independent_living_evidence']=ep;r['field_provenance']['business_license_id']=eb
def build(alis,cms,biz,il_verification=None):
 ...
 verification_by_license={str(x.get('business_license_number') or ''):x for x in ((il_verification or {}).get('records') or [])};candidates=[];seen=set();raw_candidate_rows=0;il_confirmed=il_false=il_enriched=0
 existing_key=defaultdict(list)
 for i,r in enumerate(records):existing_key[strong_key(r['facility_name'],r['address'],r['city'],r['zip'])].append(i)
 for row in biz:
  ...
  k=(norm(row.get('business_name')),norm_addr(row.get('address')),norm(row.get('city')));lic=str(row.get('license_number') or '')
  if k in seen:continue
  seen.add(k);v=verification_by_license.get(lic)
  if v and v.get('identity_verified') is True and v.get('classification')=='CONFIRMED_PRIMARY':
   vk=strong_key(v.get('business_name'),v.get('address'),v.get('city'),v.get('zip'));matches=existing_key.get(vk,[])
   if len(matches)==1:enrich_existing_with_il(records[matches[0]],v,at);il_enriched+=1
   elif len(matches)>1:conflicts.append({'business_license_id':lic,'reason':'AMBIGUOUS_IL_EXISTING_IDENTITY','candidate_count':len(matches)});candidates.append({'business_license_number':lic,'business_name':row.get('business_name') or 'UNKNOWN','address':row.get('address') or 'UNKNOWN','city':row.get('city') or 'UNKNOWN','state':row.get('state') or 'NV','zip':row.get('zip') or 'UNKNOWN','license_status':row.get('license_status') or 'UNKNOWN','classification':'INDEPENDENT_LIVING_CANDIDATE_UNKNOWN','evidence':'Primary evidence exists but canonical identity collision is ambiguous; review required.','source':ev('City of Las Vegas Business Licenses',LV_BUSINESS_URL,lic,'DISCOVERY_ENRICHMENT_ONLY',at)});continue
   else:
    new=il_record(v,at);records.append(new);existing_key[vk].append(len(records)-1)
   il_confirmed+=1;methods['INDEPENDENT_LIVING_CONFIRMED_PRIMARY']+=1;continue
  if v and v.get('identity_verified') is True and v.get('classification')=='NOT_INDEPENDENT_LIVING_PRIMARY':il_false+=1;methods['INDEPENDENT_LIVING_FALSE_POSITIVE_PRIMARY']+=1;continue
  candidates.append({'business_license_number':lic or 'UNKNOWN','business_name':row.get('business_name') or 'UNKNOWN','address':row.get('address') or 'UNKNOWN','city':row.get('city') or 'UNKNOWN','state':row.get('state') or 'NV','zip':row.get('zip') or 'UNKNOWN','license_status':row.get('license_status') or 'UNKNOWN','classification':'INDEPENDENT_LIVING_CANDIDATE_UNKNOWN','evidence':'Active Apartment House business license plus senior name signal; exact-property primary evidence remains insufficient or identity was not verified.','source':ev('City of Las Vegas Business Licenses',LV_BUSINESS_URL,lic or 'UNKNOWN','DISCOVERY_ENRICHMENT_ONLY',at)})
 groups=defaultdict(list)
 for i,r in enumerate(records):groups[r['campus_group_id']].append(i)
 multi=mixed=0
 for _,inds in groups.items():
  if len(inds)<2:continue
  multi+=1;modalities=set()
  for i in inds:modalities.update(records[i].get('facility_modalities') or [records[i]['canonical_type']])
  label='MIXED_CAMPUS' if len(modalities)>1 else 'MULTI_ENTITY_SAME_TYPE_CAMPUS';mixed+=int(label=='MIXED_CAMPUS')
 ...
 report={'generated_at':at,'canonical_facilities_unique':len(records),'nevada_official_license_records':len(alis),'cms_source_records':len(cms),'business_license_discovery_records':len(biz),'source_identity_merges':sum(v for k,v in methods.items() if k not in {'CMS_ONLY','INDEPENDENT_LIVING_CONFIRMED_PRIMARY','INDEPENDENT_LIVING_FALSE_POSITIVE_PRIMARY'}),'merge_methods':dict(methods),'unresolved_cms_identity_conflicts':[x for x in conflicts if 'ccn' in x],'unresolved_identity_conflicts':conflicts,'counts_by_type':dict(types),'memory_care_confirmed':mem,'memory_care_candidate_unknown':mem_unknown,'independent_living_confirmed':il_confirmed,'independent_living_enriched_existing_identity':il_enriched,'independent_living_false_positive_primary':il_false,'independent_living_candidate_rows_all_statuses':raw_candidate_rows,'independent_living_candidates_unknown_active_unique':len(candidates),'multi_entity_campuses':multi,'mixed_campuses':mixed,'review_flags':dict(flags),'subsets':{k:len(v) for k,v in subsets.items()},'semantic_guardrails':['Nevada HCQC/ALiS is licensing source of truth.','CMS is federal nursing-facility evidence and is never used as Nevada assisted-living licensing truth.','CMS merges require exact CCN or exact normalized name+address+city+ZIP.','Distinct Nevada licenses are never collapsed merely because they share a campus.','Memory Care requires explicit official ALiS detail evidence.','Independent Living requires exact business-license candidate identity plus primary operator/provider/property evidence.','A senior token in a business name remains UNKNOWN without primary evidence.','Primary evidence never implies nursing, assisted-living, memory-care, or other care services unless separately evidenced.','Commercial directories are not sources of licensing truth.']}
 return {'schema_version':'nevada-canonical-facility-v2.2.0','generated_at':at,'record_count':len(records),'records':records,'independent_living_discovery_candidates':candidates,'report':report,'subsets':subsets}
...
def main():
 ...
 biz=json.loads(a.business.read_text(encoding='utf-8'));il=json.loads(a.independent_living_verification.read_text(encoding='utf-8')) if a.independent_living_verification.exists() else {};p=build(alis,cms,biz,il);write(p,a.output,a.report_json,a.report_md,a.subset_dir);print(json.dumps(p['report'],indent=2,ensure_ascii=False));return 0
